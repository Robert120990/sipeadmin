...
export default function ConfiguracionDb() {
    ...
    const fetchConfig = async () => {
        try {
            const res = await api.get('/config').catch(() => ({ data: {} }));
            ...
            <div className="card glass shadow-lg" style={{ padding: '2rem' }}>
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '1rem', borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '1.5rem' }}>
                        <button type="submit" className="btn-primary" disabled={savingDB} style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', padding: '0.75rem 1.5rem' }}>
